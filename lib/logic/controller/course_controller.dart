...
class CourseController extends GetxController {
  ...
  Future getExam(int examCode) async {
    ...
    update();
    await GetMethods().getExamMethod(examCode, accessToken).then((value) {
      if (value.success==true) {


        examQuestionList = value.data;        isGettingExam = false;
        update();
      } else if(value.success==false) {
        isGettingExam = false;
        print(value.message!);
        gettingExamValidation = value.message!;
        update();
      }
